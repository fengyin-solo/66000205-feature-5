<template>
  <div class="h-screen flex flex-col">
    <!-- 顶部导航 -->
    <nav class="flex items-center gap-1 bg-gray-900 border-b border-gray-800 px-4 py-2 shrink-0">
      <span class="text-orange-400 font-bold text-sm mr-4">Modbus 工业监控</span>
      <button v-for="t in tabs" :key="t.key" @click="view = t.key"
        class="px-3 py-1.5 rounded text-xs"
        :class="view === t.key ? 'bg-orange-600 text-white' : 'text-gray-400 hover:bg-gray-800'">
        {{ t.label }}
      </button>
    </nav>

    <!-- 监控大屏 -->
    <div v-show="view === 'dashboard'" class="flex flex-1 min-h-0">
      <!-- Sidebar -->
    ...
    <!-- 其他入口 -->
    <AlarmRecords v-if="view === 'records'" />
    <ThresholdConfig v-if="view === 'rules'" />
    <RuleAudits v-if="view === 'audits'" />
  </div>
</template>
...
import RuleAudits from './components/RuleAudits.vue'
...
const tabs = [
  ...
  { key: 'rules', label: '阈值配置' },
  { key: 'audits', label: '变更记录' },
] as const
const view = ref<(typeof tabs)[number]['key']>('dashboard')
